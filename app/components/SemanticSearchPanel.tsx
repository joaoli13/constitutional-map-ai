  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          className="hidden flex-shrink-0 rounded-full border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-500 hover:text-slate-950 sm:inline-flex"
          type="button"
      ...
      <div className={`mt-3 ${isFullscreen ? "min-h-0 flex-1 flex flex-col" : ""}`}>
        {semanticSearchResults.length === 0 ? (
          ...
          <div className={`space-y-2.5 ${isFullscreen ? "min-h-0 flex-1 flex flex-col" : ""}`}>
            <div
              className={`visible-scrollbar space-y-2.5 overflow-y-scroll pr-2 ${isFullscreen ? "min-h-0 flex-1" : "max-h-[200px]"}`}
            >
